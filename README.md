...
**核心代码仅 100KB，单一Python 文件即开即用。真正的零配置、不挑设备硬件，只要安装了python的windows系统都支持，一个脚本走天下。**

不仅是个人的单兵作战神兵利器，更是**中小企业节省宝贵商业RPA经费、提升效率的有利武器**。无需购买昂贵的商业许可证，无需培训专职 RPA 工程师，一个脚本即可本地部署，一切尽在用户掌控，无需担心公司核心数据泄露，全员可用。
...
- 用 `xlwings` 操作复杂报表，再用 `auto.find()` 定位图片按钮；
- 用 `os`、`shutil` 管理文件，再用 `auto.hotkey()` 调用系统快捷键……

**入门门槛极低，却几乎看不到天花板。** Python 学得越深，自动化办公全栈能力就越牛逼————这是任何封闭式商业 RPA 语言永远无法企及的生态碾压。
...
| **彩色优先，精准如人眼** | 默认使用彩色模板匹配，能准确区分按钮的启用/禁用、链接的已读/未读状态。当然，也可以随时切回极速灰度模式。 |
| **智能去重，稳如磐石** | 内置非极大值抑制（NMS），自动合并邻近匹配点，彻底告别“一个按钮被点两次”的尴尬。 |
| **一键连点，代码再瘦身 50%** | 所有按钮截图塞进同一个文件夹，一行 `auto.click_seq(["图1","图2","图3"])` 就能依次点击。**极致精简，让多步操作像写清单一样自然。** |
| **百变按钮克星 click_any** | 同一个位置的按钮会变色、变形、换图怎么办？`auto.click_any(["蓝色按钮","红色按钮","高亮按钮"])` 按顺序尝试，命中任意一个即点击成功，**抗干扰能力拉满**。 |
